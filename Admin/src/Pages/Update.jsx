...
const Update = () => {  // Ensure id is passed as a prop or sourced from context
    const [image, setImage] = useState(null);
    const { id } = useParams();
    const [data, setData] = useState({
        topBtnText: "",
        heading: "",
        authorName: "",  // Corrected spelling
        smallClock: "",  // Corrected spelling
        timestamp: "",
        text: "",
    });
    ...
    const onSubmitHandler = async (e) => {
        ...
        formData.append("authorName", data.authorName);  // Corrected spelling
        formData.append("smallClock", data.smallClock);  // Corrected spelling
        formData.append("timestamp", data.timestamp);
        formData.append("text", data.text);
        formData.append("image", image);

        try {
            const response = await axios.put(`http://localhost:4000/api/post/update/${id}`, formData);
    ...
    return (
        <div>
            <form className="ml-2 sm:ml-10 mt-6" onSubmit={onSubmitHandler}>
                <h1 className="text-2xl font-semibold">Update The Blog:</h1>
                <label htmlFor="image">
                    <img
